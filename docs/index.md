# Manuale Operativo {{ clinic.name }}

!!! danger "AVVISO LEGALE IMPORTANTE"
    Questo Manuale Operativo costituisce **documento ufficiale** di {{ clinic.legal_name }} La consultazione e la presa visione dei contenuti creano un **obbligo contrattuale** per tutto il personale. Il mancato rispetto delle procedure documentate può costituire motivo di provvedimento disciplinare.

    **Prima di procedere**, ogni collaboratore deve:

    1. Consultare il [Sistema Documentale](sistema-documentale/index.md)
    2. Firmare il [Modulo di Presa Visione](sistema-documentale/presa-visione.md)

---

Benvenuto nel manuale operativo delle Cliniche Smiledoc. Questo documento raccoglie tutte le procedure, i protocolli e le linee guida per garantire un servizio di eccellenza.

## Sistema di Controllo Documentale

!!! info "Validità Legale Garantita"
    Questo manuale è gestito tramite un **Sistema di Controllo Documentale** conforme alla norma ISO 9001:2015 che garantisce:

    - Tracciabilità di ogni modifica
    - Integrità verificabile dei documenti
    - Non alterabilità retroattiva
    - Prova della presa visione

    [:material-shield-check: Vai al Sistema Documentale](sistema-documentale/index.md){ .md-button .md-button--primary }

---

## Struttura del Manuale

<div class="grid cards" markdown>

-   :material-earth:{ .lg .middle } **Generali**

    ---

    Manuali validi per **tutti** i collaboratori, indipendentemente dal ruolo o dalla sede.

    [:octicons-arrow-right-24: Vai ai Generali](generali/index.md)

-   :material-office-building:{ .lg .middle } **Corporate**

    ---

    Manuali per la **sede centrale** di governo e le funzioni direzionali.

    [:octicons-arrow-right-24: Vai a Corporate](corporate/index.md)

-   :material-hospital-building:{ .lg .middle } **Clinica**

    ---

    Manuali per il **funzionamento operativo** della clinica.

    [:octicons-arrow-right-24: Vai a Clinica](clinica/index.md)

-   :material-map-marker:{ .lg .middle } **Sede**

    ---

    Manuali **specifici** per ogni singola sede.

    [:octicons-arrow-right-24: Vai a Sede](sede/index.md)

</div>

---

## Legenda Contenuti

Ogni sezione contiene:

| Tipo | Descrizione |
|------|-------------|
| **Protocolli** | Regole e standard da seguire |
| **Procedure** | Passaggi operativi dettagliati |
| **Checklist** | Liste di controllo per le attività |
| **Moduli** | Modulistica e documenti |

---

## A Chi si Rivolge

| Sezione | Destinatari |
|---------|-------------|
| **Generali** | Tutti i collaboratori |
| **Corporate** | Direzione, Amministrazione centrale, HR |
| **Clinica** | Personale clinico, Segreteria, Coordinatori |
| **Sede** | Personale della specifica sede |

---

## Come Usare Questo Manuale

1. **Nuovo personale:** inizia dalla sezione [Generali](generali/index.md)
2. **Attività quotidiana in clinica:** consulta [Clinica](clinica/index.md)
3. **Informazioni specifiche della tua sede:** vai a [Sede](sede/index.md)
4. **Funzioni direzionali:** consulta [Corporate](corporate/index.md)

---

## Aggiornamenti

Questo manuale viene aggiornato regolarmente. Verificare sempre di consultare la versione più recente.

| Versione | Data | Modifiche |
|----------|------|-----------|
| 3.0 | Gennaio 2026 | Sistema Documentale con validità legale |
| 2.0 | Gennaio 2026 | Nuova struttura: Generali, Corporate, Clinica, Sede |
| 1.0 | Gennaio 2026 | Prima pubblicazione |

---

!!! warning "Obblighi del Personale"
    Ogni collaboratore è tenuto a:

    - Consultare regolarmente questo manuale
    - Firmare il modulo di presa visione
    - Rispettare tutte le procedure documentate
    - Segnalare eventuali non conformità

    La consultazione di questo manuale e la firma della presa visione costituiscono **prova documentale** della conoscenza delle procedure aziendali.

---

*Per segnalare errori o proporre miglioramenti, contattare il Responsabile Qualità.*

---

**{{ clinic.legal_name }}** | {{ clinic.full_address }} | Sistema Documentale Conforme ISO 9001:2015
